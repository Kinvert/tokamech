...
static void test_run_config_reads_project_model_and_train_sections(void) {
    ...
    CHECK(config.layers.model == TKM_MODEL_ACTION_SCORER);
    CHECK(config.layers.model_hidden_dim == 64);
    CHECK(config.layers.action_features == TKM_ACTION_FEATURE_FOOD_SPACE);
    CHECK(config.train.loss == TKM_TRAIN_LOSS_MASKED_CROSS_ENTROPY);
    ...
    CHECK(config.mask.strategy == TKM_MASK_STRATEGY_SURVIVAL);
    CHECK(config.decoder.fallback == TKM_DECODER_FALLBACK_ROLLOUT_SCORE);
    CHECK(config.decoder.rollout_horizon == 24);
    CHECK(config.decoder.rollout_score_mode == TKM_DECODER_ROLLOUT_SCORE_SPACE_DISTANCE);
}
...
static void test_run_config_reads_project_local_ini_file(void) {
    ...
    CHECK(config.layers.action_features == TKM_ACTION_FEATURE_SPACE);
    CHECK(config.train.loss == TKM_TRAIN_LOSS_MARGIN);
    ...
    CHECK(config.decoder.rollout_score_mode == TKM_DECODER_ROLLOUT_SCORE_FOOD_STEPS);
}
...
static void test_run_config_rejects_bad_inputs(void) {
    ...
    CHECK(tkm_run_config_from_file("projects/snake/action_scorer.ini", 0) == TKM_ERR);
}
