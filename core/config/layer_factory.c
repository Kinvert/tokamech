...
int tkm_layer_factory_init_mlp_window(const TkmIni* ini, const TkmParams* params, TkmMlpWindow* out) {
    ...
    if (tkm_layer_config_from_ini(ini, &config) != TKM_OK || config.model != TKM_MODEL_MLP_WINDOW) {
        return TKM_ERR;
    }
    if (tkm_layer_factory_required_i32(ini, "model", "input_dim", &input_dim) != TKM_OK ||
        tkm_layer_factory_required_i32(ini, "model", "hidden_dim", &hidden_dim) != TKM_OK ||
        tkm_layer_factory_required_i32(ini, "model", "output_dim", &output_dim) != TKM_OK ||
        input_dim <= 0 ||
        hidden_dim <= 0 ||
        output_dim <= 0) {
        return TKM_ERR;
    ...
    w0_name = tkm_layer_factory_required_string(ini, "model", "w0");
    b0_name = tkm_layer_factory_required_string(ini, "model", "b0");
    w1_name = tkm_layer_factory_required_string(ini, "model", "w1");
    b1_name = tkm_layer_factory_required_string(ini, "model", "b1");
    if (!w0_name || !b0_name || !w1_name || !b1_name) {
        return TKM_ERR;
    }
    if (tkm_layer_factory_get_param(params, w0_name, (uint32_t)input_dim * (uint32_t)hidden_dim, &w0) != TKM_OK ||
        tkm_layer_factory_get_param(params, b0_name, (uint32_t)hidden_dim, &b0) != TKM_OK ||
        tkm_layer_factory_get_param(params, w1_name, (uint32_t)hidden_dim * (uint32_t)output_dim, &w1) != TKM_OK ||
        tkm_layer_factory_get_param(params, b1_name, (uint32_t)output_dim, &b1) != TKM_OK) {
        return TKM_ERR;
    }
...
int tkm_layer_factory_init_action_scorer(const TkmIni* ini, const TkmParams* params, TkmActionScorer* out) {
    ...
    if (tkm_layer_config_from_ini(ini, &config) != TKM_OK || config.model != TKM_MODEL_ACTION_SCORER) {
        return TKM_ERR;
    }
    if (tkm_layer_factory_required_i32(ini, "model", "state_dim", &state_dim) != TKM_OK ||
        tkm_layer_factory_required_i32(ini, "model", "action_dim", &action_dim) != TKM_OK ||
        tkm_layer_factory_required_i32(ini, "model", "hidden_dim", &hidden_dim) != TKM_OK ||
        state_dim <= 0 ||
        action_dim <= 0 ||
        hidden_dim <= 0) {
    ...
    input_dim = (uint32_t)state_dim + (uint32_t)action_dim;
    w0_name = tkm_layer_factory_required_string(ini, "model", "w0");
    b0_name = tkm_layer_factory_required_string(ini, "model", "b0");
    w1_name = tkm_layer_factory_required_string(ini, "model", "w1");
    b1_name = tkm_layer_factory_required_string(ini, "model", "b1");
    ...
    if (tkm_layer_factory_get_param(params, w0_name, input_dim * (uint32_t)hidden_dim, &w0) != TKM_OK) {
        return TKM_ERR;
    }
    if (tkm_layer_factory_get_param(params, b0_name, (uint32_t)hidden_dim, &b0) != TKM_OK ||
        tkm_layer_factory_get_param(params, w1_name, (uint32_t)hidden_dim, &w1) != TKM_OK ||
        tkm_layer_factory_get_param(params, b1_name, 1u, &b1) != TKM_OK) {
        return TKM_ERR;
    }
